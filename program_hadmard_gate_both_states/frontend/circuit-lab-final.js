...
makeResizable(el.sidebarResizer, () => sidebarW, w => { sidebarW = w; }, 160, 480);
makeResizable(el.resultsResizer, () => resultsW, w => { resultsW = w; }, 160, 560);
  ...
  el.circuitBoard.querySelectorAll('[data-gate-id]').forEach(btn => {
    ...
    if (handle) {
      handle.addEventListener('mousedown', e => {
        ...
        const g = circuit.find(item => item.id === id);
        if (!g) return;
        const startQubit = g.target, col = g.column;
        ...
        function onMove(mv) {
          ...
          const lo = Math.min(startQubit, endQubit), hi = Math.max(startQubit, endQubit);
        ...
        function onUp() {
          ...
          if (endQubit === startQubit) return;
          const lo = Math.min(startQubit, endQubit), hi = Math.max(startQubit, endQubit);
          snapshot();
          for (let q = lo; q <= hi; q++) {
            if (q === startQubit) continue;
            circuit = circuit.filter(item => !(item.column === col && item.target === q));
            circuit.push({ id: Date.now()+Math.random(), gate: g.gate, column: col, target: q, control: -1, control2: -1, angle: g.angle });
          }
          renderBoard(); updateMeta();
        }
        document.addEventListener('mousemove', onMove);
...
function applyBulk() {
  ...
  if (fromQ > toQ) [fromQ, toQ] = [toQ, fromQ];

  const mode   = el.bulkCol.value;
  ...
  if (mode === 'first-empty') {
    qubits.forEach(q => {
      ...
      let col = 0;
      while (occupied.has(col)) col++;
      circuit.push({ id: Date.now()+Math.random(), gate: gateName, column: col, target: q, control: -1, control2: -1, angle: ROTATION.has(gateName) ? Math.PI/2 : 0 });
    });
  } else {
    let col = 0;
    while (circuit.some(g => g.column === col && g.target >= fromQ && g.target <= toQ)) col++;
    qubits.forEach(q => {
      circuit.push({ id: Date.now()+Math.random(), gate: gateName, column: col, target: q, control: -1, control2: -1, angle: ROTATION.has(gateName) ? Math.PI/2 : 0 });
    });
  }
  ...
  setTimeout(() => { el.bulkApplyBtn.textContent = '⚡ Apply to all'; el.bulkApplyBtn.style.background = ''; }, 1800);
}

function syncBulkRange() {
  const max = Number(el.qubitCount.value) - 1;
  el.bulkFrom.max = max; el.bulkTo.max = max;
